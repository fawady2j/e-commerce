...
const BikeCategories = ({ bikes }) => {
  ...
  useEffect(() => {
    const filtered = bikes.filter((bikes) => {
      const categoryMatch =
        category === "all"
          ? bikes
          : bikes.categories.some((categ) => categ.name === category);
      const priceMatch = bikes.price <= price;
          ...
          <aside className="w-full p-4 mb-8 xl:w-[300px] xl:h-[84vh] xl:fixed">
            ...
            <div className="max-w-56">
              <div className="text-lg mb-4 font-medium">
                Max Price:{" "}
                <span className="text-primary-accent font-semibold ml-2">
                  ${price}
                </span>
                <span className="ml-2">
                  (
